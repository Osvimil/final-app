...
import com.appspot.cursosprogramacioncu.proxyEndpointCurso.model.PdfCollection;
...
public class FragmentPdfs extends Fragment
        implements LoaderManager.LoaderCallbacks<Respuesta<PdfCollection>> {
    private static final String TAG = FragmentPdfs.class.getName();
    private String mensaje;
    @Nullable private AdapterPdf adapter;
    ...
    @Override public Loader<Respuesta<PdfCollection>> onCreateLoader(int id,
                                                                     Bundle args) {
        setIndicadorActivo(indicador, true);
        final App app = getApp(getActivity());
        return new AsyncTaskLoader<Respuesta<PdfCollection>>(app) {
            @Override protected void onStartLoading() {
                forceLoad();
            }
            @Override public Respuesta<PdfCollection> loadInBackground() {
                try {
                    final ProxyEndpointCurso.ListPdf list =
                            app.getProxyEndpointConocidos().listPdf();
                    final PdfCollection lista = list.execute();
    ...
    @Override
    public void onLoadFinished(Loader<Respuesta<PdfCollection>> loader,
                               Respuesta<PdfCollection> data) {
    ...
    @Override
    public void onLoaderReset(Loader<Respuesta<PdfCollection>> loader) {
    }
