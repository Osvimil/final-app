...
import com.appspot.cursosprogramacioncu.proxyEndpointCurso.model.CursoPremiumCollection;
...
public class FragmentCursosPremium extends Fragment
        implements LoaderManager.LoaderCallbacks<Respuesta<CursoPremiumCollection>> {
    private static final String TAG = FragmentCursosPremium.class.getName();
    private String mensaje;
    @Nullable
    private AdapterCursosPremium adapter;
    @Nullable
    ...
    public void onViewCreated(View view, @Nullable Bundle savedInstanceState) {
        ...
        adapter = new AdapterCursosPremium();
    ...
    @Override public Loader<Respuesta<CursoPremiumCollection>> onCreateLoader(int id,
                                                                       Bundle args) {
        setIndicadorActivo(indicador, true);
        final App app = getApp(getActivity());
        return new AsyncTaskLoader<Respuesta<CursoPremiumCollection>>(app) {
            @Override protected void onStartLoading() {
                forceLoad();
            }
            @Override public Respuesta<CursoPremiumCollection> loadInBackground() {
                try {
                    final ProxyEndpointCurso.ListPremium list =
                            app.getProxyEndpointConocidos().listPremium();
                    final CursoPremiumCollection lista = list.execute();
    ...
    @Override
    public void onLoadFinished(Loader<Respuesta<CursoPremiumCollection>> loader,
                               Respuesta<CursoPremiumCollection> data) {
    ...
    @Override
    public void onLoaderReset(Loader<Respuesta<CursoPremiumCollection>> loader) {
    }
